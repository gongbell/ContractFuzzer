...
func run(evm *EVM, snapshot int, contract *Contract, input []byte) ([]byte, error) {
	...
	/**
	*添加hacker初始环境代码
	* hacker_env :hacker中需要使用的环境变量,evm的别名
	* hacker_call_stack:hacker中的合约的调用栈,记录合约调用关系及其输入
	* 					初始为空栈
	**/
...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	// fmt.Printf("caller:%s,callee:%s", caller.Address().Hex(), addr.Hex())
	defer func() { // 必须要先声明defer，否则不能捕获到panic异常
		if err := recover(); err != nil {
			fmt.Println("error happened in Evm.Call")
			log.Printf("%v",err) // 这里的err其实就是panic传入的内容，55
			for i := 0; i < 10; i++ {
	...
	if caller != nil&&!isRelOracle(contract.Address()) {
		// fmt.Printf("report from evm.Call")
		// ccall := CreateAndGetCCall(evm, caller, addr, input, gas, value, false)
		// subcriber := NewChangeSubscriber(ccall)
		// subcriber.Open()
		// subcriber.OnCall()
		/***
		*记录CALL操作并且创建新的HackerContractCall
		*将新的CALL操作压栈
		**/
		hacker_init(evm, contract, input)
	...
	if caller != nil&&!isRelOracle(contract.Address()) {
		...
		// subcriber.Write()
		/**
		*CALL操作结束,hacker_call_stack 执行出栈操作,赋值call
		*call完成CloseCall动作,记录最终状态
		**/
		if hacker_call_stack != nil {
...
func (evm *EVM) CallCode(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if caller != nil&&!isRelOracle(contract.Address()) {
		...
		// subcriber.Write()
		/**
		*CALLCODE操作结束,hacker_call_stack 执行出栈操作,赋值call
		*callcode完成CloseCall动作,记录最终状态
		**/
		if hacker_call_stack != nil {
			call := hacker_call_stack.pop()
			call.nextRevisionId = nextRevisionId
...
func (evm *EVM) DelegateCall(caller ContractRef, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
	...
	if caller != nil&&!isRelOracle(contract.Address()) {
		...
		// subcriber.Write()
		/**
		*DelegateCALL操作结束,hacker_call_stack 执行出栈操作,赋值call
		*delegatecall完成CloseCall动作,记录最终状态
		**/
		if hacker_call_stack != nil {
			call := hacker_call_stack.pop()
			call.nextRevisionId = nextRevisionId
